...
* including provision of callbacks that notify the application when a message arrives from or is delivered 
...
/*
 * GETBROKER FUNCTION - added by CATA to export the connected broker from the list
 */

ClientImpl.prototype.getBroker = function () 
{
	//console.log("CATA - getBroker" + this.cata_connected_host);
	return [this.cata_connected_host, this.cata_connected_port, this.cata_connected_ssl];
	//return this.connectOptions.hosts[this.hostIndex];
};
...
{
 //console.log("CATA: connect options");
 ...
 {
  this.hostIndex = 0;
  this._doConnect(connectOptions.hosts[0], connectOptions.ports[0], connectOptions.useSSL[0]);
 } 
 else 
 {
...
ClientImpl.prototype._doConnect = function (host, port,ssl) 
{
// When the socket is open, this client will send the CONNECT WireMessage using the saved parameters.
//if (this.connectOptions.useSSL) //BCD orig
console.log("Connecting to: " + host +" port:" + port + " ssl:"+ssl);
if (ssl)
...
this._connectTimeout = new Timeout(this, window, this.connectOptions.timeout, 
                                   this._disconnected, 
...
ClientImpl.prototype._on_socket_message = function (event) 
{
  ...
  switch (wireMessage.type) 
  {
    case MESSAGE_TYPE.CONNACK:
      ...
      {
        this.connected = true;
        //CATA new code to find on wich broker I am connected
        this.cata_connected_host = this.connectOptions.hosts[this.hostIndex];
        this.cata_connected_port = this.connectOptions.ports[this.hostIndex];
        this.cata_connected_ssl  = this.connectOptions.useSSL[this.hostIndex];
        
        //console.log("CATA: _on_socket_message - connected:");
        //console.log("CATA: _on_socket_message host:" + this.connectOptions.hosts[this.hostIndex] + " port:" + this.connectOptions.ports[this.hostIndex] + " ssl:"+ this.connectOptions.useSSL[this.hostIndex] );
...
ClientImpl.prototype._disconnected = function (errorCode, errorText) {
 ...
 {
  // Try the next host.
  //console.log("CATA: _disconnect try new broker");
  this.hostIndex++;
  this._doConnect(this.connectOptions.hosts[this.hostIndex], this.connectOptions.ports[this.hostIndex], this.connectOptions.useSSL[this.hostIndex]);
  //console.log("CATA: _disconnect host:" + this.connectOptions.hosts[this.hostIndex] + " port:" + this.connectOptions.ports[this.hostIndex] + " ssl:"+ this.connectOptions.useSSL[this.hostIndex] );
 } 
 else 
 {
...
//CATA add function to retrieve the connected host
this.getConnectedBroker = function()
{
	//console.log("CATA: getConnectedBroker");
	return client.getBroker();
	//return this.cata_connected_host = this.connectOptions.hosts[this.hostIndex];
};
